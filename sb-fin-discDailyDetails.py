...
def convert_types_discdailydetails(data):
    ...
    data["percent"] = float(data['percent']) if data.get("percent", "") != ""  else None
    data['total'] = abs(float(data['total'])) if 'total' in data else None
    data['percent_of_total'] = abs(float(data['percent_of_total'])) if 'percent_of_total' in data else None
    data['count'] = int(data['count']) if 'count' in data else None
    data['average'] = abs(float(data['average'])) if 'average' in data else None
...
# with beam.Pipeline(options=PipelineOptions) as p:
    ...
    # Write to BQ
    # DiscDailyDetails_data | 'Write to BQ Tender Daily' >> beam.io.WriteToBigQuery(

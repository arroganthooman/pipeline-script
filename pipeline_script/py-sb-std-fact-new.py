...
# CLASS REMOVE EMPTY PLU FROM DAILY ORDER
class remove_empty_plu(beam.DoFn):
...
# CLASS CHANGE PRICE AMOUNT FOR PLU SCCRM01
class update_price_sccrm01(beam.DoFn):
    def process(self, data):
        try:
            if data['plu'] == 'SCCRM01':
                data['price'] = data['stdetail_net_price']
                yield data
            else: yield data
        except:
            yield data
...
class dodt_exclude_zero_price(beam.DoFn):
    def process(self, data):
        try:
            ...
            data['price'] = float(0)
            data['dorder_upref'] = ''
            data['dorder_receiveon'] = data['receiveOn']
            yield data
...
class remove_dtdo_price(beam.DoFn):
    def process(self, data):
        data.pop("price", None)
        data.pop("qty",None)
        yield data

class check_net_price_price(beam.DoFn):
    def process(self, data):
        if data['stdetail_net_price'] == 0 and data['price'] ==0:
            yield data
        elif data['stdetail_net_price'] != 0 and data['price'] !=0:
            if data['stdetail_net_price'] == data['price']:
                yield data
            elif data['stdetail_net_price'] < data['price']:
                if data['stdetail_extradisc_amt'] != 0:
                    yield data
                else: return
...
def convert_types_sales_transaction(data):
    ...
    data['st_transaction_date'] = str(st_trans_date.date())
...
def convert_types_sales_transaction_details(data):
    ...
    data['stdetail_qty'] = float(data['stdetail_qty']) if 'stdetail_qty' in data else None                                                          # FACT
...
def convert_types_daily_table(data):
    ...
    data['sales_date'] = str(dtable_sales_date.date())
    ...
    data['transaction_number'] = str(data['transaction_number']) if 'transaction_number' in data else None
    data['branch_id'] = str(data['branch_id']) if 'branch_id' in data else None
    receiveOn = datetime.datetime.strptime(data['receiveOn'],date_format)
    data['receiveOn'] = str(receiveOn.date())
    return data
...
def convert_types_daily_order(data):
    ...
    data['sales_date'] = str(dorder_sales_date.date())
    data['cash_register_id'] = str(data['cash_register_id']) if 'cash_register_id' in data else None
    data['order_id'] = int(data['order_id']) if 'order_id' in data else None
...
def del_unwanted_cols_std_fact(data):
    del data['dorder_receiveon']
...
table_id_fact_stdetail = 'ivan_fact_stdetail_test_1D'
...
def run(argv=None):
    ...
    daily_order_data = (p | 'ReadData daily order' >> beam.io.ReadFromText(dorder, skip_header_lines =1, coder=IgnoreUnicode())
                            ...
                            | 'Remove Empty PLU in DAILY ORDER' >> beam.ParDo(remove_empty_plu())
    ...
    # DISTINCT METHOD
    rm_transtypemicros_data = (transtypemicros_data | 'CHANGE TRANS TYPE MICROS INTO LIST' >> beam.Map(lambda x: str(x))
                                                    | 'DISTINCT TRANS TYPE MICROS' >> beam.Distinct()
                                                    | 'EVALUATE LIST TRANS TYPE MICROS' >> beam.Map(lambda x: eval(x))
    )
    ...
    def add_to_list(data):
        global STD_DTDO
        STD_DTDO.append(data)


    global STD_DTDO_NO_DUP
    STD_DTDO_NO_DUP = set()

    def remove_duplicate_trans_seq(data):
        global STD_DTDO
        for row in range(len(STD_DTDO)):
            is_same_trans_number = STD_DTDO[row].get("transaction_number") == data.get("transaction_number")
            is_same_seq = STD_DTDO[row].get("stdetail_seq") == data.get("stdetail_seq")

            if is_same_trans_number and is_same_seq:
                data['dorder_upref'] = data['dorder_upref'] or STD_DTDO[row]['dorder_upref']
                data['dorder_text'] = data['dorder_text'] or STD_DTDO[row]['dorder_text']
                global STD_DTDO_NO_DUP
                STD_DTDO_NO_DUP.add(str(data))
    ...
    left_join_std_dtdo = (join_stddtdo_dict | 'Left Join STD DTable DOrder' >> Join(left_pcol_name=stdbsap_dimm_data, left_pcol=left_join_std_bsap,
                                            ...
                                            # | 'CHECK stdetail_net_price WITH price' >> beam.ParDo(check_net_price_price())
                                            | 'REMOVE PRICE from DICTIONARY' >> beam.ParDo(remove_dtdo_price())
                                            | 'ADD TO STD_DTDO LIST' >> beam.Map(add_to_list)
                                            | 'REMOVE DUPLICATE and ADD TO LIST' >> beam.Map(remove_duplicate_trans_seq)
                                            | 'WriteToText STDETAIL >< DTDO' >> beam.io.WriteToText("test_join_duplicate", ".txt")
    )

    p.run().wait_until_finish()

    # CREATE FROM STD_DTDO_NO_DUP LIST
    left_join_std_dtdo = (p | 'Create from STD_DTDO_NO_DUP' >> beam.Create(list(STD_DTDO_NO_DUP))
                        | "ConvertStrToDict" >> beam.Map(lambda data: eval(data))
                        | "WriteToText STDETAIL >< DTDO" >> beam.io.WriteToText("test_join_no_duplicate", ".txt")
                        )


    
    
    print(left_join_std_dtdo)
    ...
    # DISTINCT METHOD
    rm_left_std_dtdo = (left_join_std_dtdo | 'CHANGE JOINED STD DTDO INTO LIST' >> beam.Map(lambda x: str(x))
                                            | 'DISTINCT STD DTDO' >> beam.Distinct()
                                            | 'EVALUATE LIST STD DTDO' >> beam.Map(lambda x: eval(x))
    ...
    stdetail_fact = (left_join_stdbaspdtdomtpd  | 'toLIST STDETAIL FACT' >> beam.Map(lambda x: str(x))
                                                | 'Distinct STDETAIL FACT' >> beam.Distinct()
                                                | 'EVALUATE LIST STDETAIL FACT' >> beam.Map(lambda x: eval(x))
    )
    ...
    # left_join_dtdo | 'WriteToText DODT' >> beam.io.WriteToText('data_output/12082021_5-2_dodt_data','.txt')
    ...
    # rm_left_std_dtdo | 'WriteToText STD BASP PAID DTDO FILTERED REMOVE' >> beam.io.WriteToText('data_output/12082021_3_std_dtdo_data','.txt')
    ...
    # stdetail_fact | 'WriteToText STDETAIL FACT' >> beam.io.WriteToText('data_output/13082021_1_stdetail_fact','.txt')
    ...
    # stdetail_fact | 'WriteToBigQuery STDETAIL FACT' >> beam.io.WriteToBigQuery(
    #                             table_spec_std_fact,
